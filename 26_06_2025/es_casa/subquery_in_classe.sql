...
-- 2) Selezionare il codice dei dipendenti che hanno avuto lo stipendio maggiore di 
    ...
    from stipendio s
    where s.stipendio > all(
    select max(s1.stipendio)
    from stipendio s1
    where s1.dip_id = 10100);
...
select d.dip_id, d.cognome, d.nome, s.stipendio
from dipendente d
join stipendio s on s.dip_id = d.dip_id
where s.stipendio >= all(
select s1.stipendio
from stipendio s1
where s1.dip_id = d.dip_id)
order by d.cognome, d.nome, s.stipendio desc;
...
from dipendente d
join stipendio s on s.dip_id = d.dip_id
where s.stipendio > 50000; -- esegue separatamente e poi fa la sottrazione, meno efficiente
...
select*
from stipendio s1
where s1.stipendio > s.stipendio);
